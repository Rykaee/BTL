...
public class HogController : MonoBehaviour
{
    ...
    public bool attackTimerIsRunning = false;
    ...
    {   //If the enemy has not been knocked back
        if (knockBackCounter <= 0)
        {
            ...
            if (distance < 5 && enemyRB.position.x > leftPoint.position.x && enemyRB.position.x < rightPoint.position.x) //Only follow between points
            {
                moveSpeed = 0;
                ...
                attackTimerIsRunning = true; //Start timer for attack
                if (attackTimer > 0)
                {
                ...
                {
                    moveSpeed = 7.5f; //Charge at the player after 0.6
                    attackTimerIsRunning = false;
                }
            ...
            else
            {
                ...
                attackTimer = 0.7f; // Reset attack timer
